...
namespace iBarter {
    public class CFunctions {
        ...
        private static bool TryBuildItem2OcrRectangle(
            ...
            // A wrapped item name can leave a single glyph on the second line
            // (for example, 阿利赫兹灯塔雕 / 像). Keep one additional line of
            // vertical breathing room so anti-aliased glyphs at the crop edge
            // do not cause Tesseract to discard the structured first line.
            y2 = Math.Max(y1, parleyY + 2 * parleyHeight);
...
// rescued a case where A + R + G already agreed. -1 outcomes stayed -1.)
        ...
        private Barter IdentifyBarterAsync(PointPlus _pp) {
            ...
            // Use the same generous wrapped-name rectangle for every slot2.
            // Auto segmentation remains the primary path for ordinary names;
            // sparse segmentation below handles a one-glyph second line.
            if (!TryBuildItem2OcrRectangle(
                    ...
                    out int item2X2,
                    out int item2Y2)) {
                Log("[DIAG-item-ocr] invalid wrapped slot2 rectangle", Brushes.IndianRed);
                return (Barter)null;
            }
            ...
            // Auto can keep only the isolated second-line glyph and discard
            // the tiered first line (observed: "像" became "人" for
            // 阿利赫兹灯塔雕像). SparseText inspects both visual lines. Prefer
            // whichever read preserves a recognizable tier prefix.
            ...
            // SparseText can preserve line breaks. The catalog stores the same
            // name as one logical line, so join only newline boundaries while
            // preserving ordinary English spaces.
