...
**Architecture:** 路线状态机是唯一的库存/载重真相；航行走廊服务是距离和渲染折线的共同真相；协调器负责验证后的计划持久化及 UI 选择状态。优化器只生成候选，最终必须由 `RoutePlanVerifier` 全量回放确认。
...
2. 写测试证明渲染快照包含同一组中间导航点，而非只有任务岛 ID。
3. 实现不可变小型走廊图及折线长度；将状态转换中的距离改为该服务。
4. 扩展地图绘制为折线，右上中间点通过现有 inset 坐标系投影。
...
3. XAML 用 ItemsControl 展示图标与名称数量；删除原始 ID 拼接详情。
4. 切换语言时重建步骤 VM 并刷新。
