...
namespace iBarter
{
    public sealed class WebP : IDisposable
    {
        ...
        public Bitmap Decode(byte[] rawWebP, WebPDecoderOptions options, PixelFormat pixelFormat = PixelFormat.DontCare)
        {
            ...
            try
            {
                ...
                if (result != VP8StatusCode.VP8_STATUS_OK) throw new Exception("Failed WebPDecode with error " + result);
        ...
        /// <summary>
        ///     Compute PSNR, SSIM or LSIM distortion metric between two pictures. Warning: this function is rather
        ///     CPU-intensive
        /// </summary>
    ...
    internal sealed class UnsafeNativeMethods
    {
        ...
        /// <summary>
        ///     Initialize the configuration as empty. This function must always be called first, unless WebPGetFeatures() is
        ///     to be called
        /// </summary>
    ...
    internal struct WebPConfig
    {
        ...
        /// <summary>
        ///     Quality degradation allowed to fit the 512k limit on prediction modes coding (0: no degradation, 100: maximum
    ...
    internal struct WebPPicture
    {
        ...
        /// <summary>
        ///     If not NULL, points to an array of size ((width + 15) / 16) * ((height + 15) / 16) that will be filled with a
        ///     macroblock map, depending on extra_info_type
        /// </summary>
    ...
    internal struct WebPDecBuffer
    {
        ...
        /// <summary>
        ///     If non-zero, 'internal_memory' pointer is not used. If value is '2' or more, the external memory is considered
        ///     'slow' and multiple read/write will be avoided
        /// </summary>
